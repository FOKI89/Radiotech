<?php
	...
	*/
	function radiotech_setup() {
		 ...
		 * If you're building a theme based on radiotech-master, use a find and replace
		 * to change 'radiotech-master' to the name of your theme in all the template files
		 */
		load_theme_textdomain( 'radiotech', get_template_directory() . '/languages' );

		radiotech_setupThemeSupport();
	...
	add_action( 'after_setup_theme', 'radiotech_setup' );
	 ...
	 */
	function radiotech_setupThemeSupport(){
		...
		*/
		...
		add_theme_support( 'custom-background', apply_filters( 'radiotech_custom_background_args', array(
	 ...
	 */
	function radiotech_setupPostSupport(){
	 ...
	 */
	if (!function_exists("radiotech_setupImageFormats")) {

		function radiotech_setupImageFormats(){
			add_filter( 'image_size_names_choose', 'radiotech_addImageSizesToSelector' );
		}
	 ...
	 * @return multitype:
	 */
	if (!function_exists("radiotech_addImageSizesToSelector")) {

		function radiotech_addImageSizesToSelector( $sizes ) {
			$custom_sizes = array(
	...
	*/
	function radiotech_enqueueScripts() {
